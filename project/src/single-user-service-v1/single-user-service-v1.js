//single user version 1: only returns id, firstname, lastname
const port = 3001;
...
app.get('/', async(req, res) => {
  ...
  res.send("hello world - singleuser-v1 service");

})

app.listen(port, function(){
    console.log(`single-user-service-v1 listening on port ${port}`);
})

//Get the tracing headers and return a header object
...
// v1 - first_name, last_name, service_name
function getUserDetail(users, id){
  let user;
  for(let u of users){
    if(u.id = id){
       user = {user: {id: u.id, first_name: u.first_name, last_name: u.last_name}, service: 'single-user-service-v1'};
    }
  }
  return user;
}
